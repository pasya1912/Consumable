...
@foreach($arr['items'] as $key =>$item)
  <tr>
    ...
    <td style="text-align: center">
        <input class="form-control" id="inputRequest{{$key}}" type="number" onchange="ubahJumlah({{$key}},{{$item->remaining_quota}})" value="{{$arr['cart'][$key]['jumlah']}}"/> Max: {{$item->remaining_quota       }}   {{$item->satuan_oca}}
    </td>
    <td style="text-align: center">
