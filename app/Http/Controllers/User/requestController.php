    ...
    function index(Request $request)
    {
        ...
        foreach ($cart_items as $key => $value) {
            $item = DB::table('item_master')
                ...
                ->where('item_master.code_item', $value)
                ->where('budget.user', $request->user()->username)
                ->first();
            $items[] = $item;
            $items[$key]->remaining_quota = $item->quota;
            foreach ($req as $key2 => $value2) {
                if ($item->code_item == $value2->code_item) {
                    $items[$key]->remaining_quota = $item->quota - $value2->qty;
                }
            }
        }
